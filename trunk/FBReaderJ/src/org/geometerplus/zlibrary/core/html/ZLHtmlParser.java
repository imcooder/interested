...
final class ZLHtmlParser {
	...
	private static ZLByteBuffer unique(
			HashMap<ZLByteBuffer, ZLByteBuffer> strings, ZLByteBuffer container) {
	...
	public ZLHtmlParser(ZLHtmlReader htmlReader, InputStream stream)
			throws IOException {
		myReader = htmlReader;
		myStream = stream;
	}
			...
			try {
				for (int i = -1;;) {
						...
						while (true) {
							...
							case '>': {
								...
								tagName.append(buffer, startPosition, i
										- startPosition);
								{
									ZLByteBuffer stringTagName = unique(
											strings, tagName);
									processStartTag(htmlReader, stringTagName,
											offset, attributes);
									if (stringTagName
											.equalsToLCString("script")) {
										scriptOpened = true;
										state = SCRIPT;
										break mainSwitchLabel;
									}
							...
							case '/':
								...
								tagName.append(buffer, startPosition, i
										- startPosition);
								// processFullTag(htmlReader, unique(strings,
								// tagName), attributes);
								break mainSwitchLabel;
							}
						}
					...
					case WS_AFTER_START_TAG_NAME:
						switch (buffer[++i]) {
						case '>': {
							ZLByteBuffer stringTagName = unique(strings,
									tagName);
							processStartTag(htmlReader, stringTagName, offset,
									attributes);
					...
					case ATTRIBUTE_NAME:
						while (true) {
							switch (buffer[++i]) {
							case '=':
								attributeName.append(buffer, startPosition, i
										- startPosition);
					...
					case DEFAULT_ATTRIBUTE_VALUE:
						while (true) {
							...
							if ((buffer[i] == ' ') || (buffer[i] == '\'')
									|| (buffer[i] == '"') || (buffer[i] == '>')) {
								attributeValue.append(buffer, startPosition, i
										- startPosition);
								attributes.put(unique(strings, attributeName),
										new ZLByteBuffer(attributeValue));
					...
					case SLASH:
						while (true) {
							...
							case ' ':
								break;
							case '>': {
								processFullTag(htmlReader,
										unique(strings, tagName), offset,
										attributes);
								state = TEXT;
								startPosition = i + 1;
							}
					...
					case TEXT:
						while (true) {
							...
							case '<': {
								if (i > startPosition) {
									htmlReader.byteDataHandler(buffer,
					...
					case ENTITY_REF:
						while (true) {
							byte sym = buffer[++i];
							if (sym == ';') {
								entityName.append(buffer, startPosition, i
										- startPosition);
								state = TEXT;
								startPosition = i + 1;
								htmlReader.entityDataHandler(unique(strings,
										entityName).toString());
								entityName.clear();
								break mainSwitchLabel;
							} else if ((sym != '#')
									&& !Character.isLetterOrDigit(sym)) {
								entityName.append(buffer, startPosition, i
										- startPosition);
								state = TEXT;
								startPosition = i;
								htmlReader.byteDataHandler(new byte[] { '&' },
										0, 1);
								htmlReader.byteDataHandler(entityName.myData,
										0, entityName.myLength);
								entityName.clear();
								break mainSwitchLabel;
							}
						}
				...
				case END_TAG:
					...
					break;
				case ATTRIBUTE_NAME:
					attributeName.append(buffer, startPosition, count
							- startPosition);
					break;
				case S_ATTRIBUTE_VALUE:
				case D_ATTRIBUTE_VALUE:
					attributeValue.append(buffer, startPosition, count
							- startPosition);
					break;
				case TEXT:
					htmlReader.byteDataHandler(buffer, startPosition, count
							- startPosition);
					break;
				case ENTITY_REF:
					entityName.append(buffer, startPosition, count
							- startPosition);
					break;
				}
			}
	...
	private static void processStartTag(ZLHtmlReader htmlReader,
			ZLByteBuffer tagName, int offset, ZLHtmlAttributeMap attributes) {
		htmlReader.startElementHandler(tagName.toString(), offset, attributes);
		attributes.clear();
	}

	private static void processEndTag(ZLHtmlReader htmlReader,
			ZLByteBuffer tagName) {
