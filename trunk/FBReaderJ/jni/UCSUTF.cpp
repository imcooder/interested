...
int Conv_UTF8_To_UCS4( const unsigned char* pUTF8Str, unsigned long* pdwCode )
{
	...
	if( b < 0xC0 || b > 0xFD )return 0; /* �Ƿ�UTF8 */
...
long UTF16ToUTF8(LPCWSTR pszUTF16, char* pszUTF8)
{		
	if (!pszUTF16)
	{
		return -1;
...
long UTF8ToUTF16(const CHAR *pszUTF8, LPWSTR pszUTF16)
{
	if (!pszUTF8)
	{
		return -1;
...
/* ת��UTF16���뵽UCS4���� */
long UTF16ToUTF32( LPCWSTR pUTF16Str, LPDWORD pdwCode )
{
	WCHAR w1 = 0, w2 = 0;
...
LPWSTR HWCharNext( LPCWSTR pszStr)
{
...
LPWSTR HWStrLocate(LPCWSTR pszStr, DWORD dwIdx)
{
